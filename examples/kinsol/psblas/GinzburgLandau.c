...
struct user_data_for_f {
 ...
 int main(int argc, char *argv[]){
   ...
   psb_c_ibcast(*cctxt,1,&dumpflag,0);
   ...
   if (info=psb_c_cdall_nl(nr, *cctxt, cdh)!=0) {
   ...
   for(int ii = 0; ii < nlr; ii += nb){
    ...
    if (check_flag(&info, "KINSol", 1, iam)) psb_c_abort(*cctxt);
    t2 = psb_c_wtime();
...
static int funcprpr(N_Vector u, N_Vector fval, void *user_data)
{
  ...
  N_Vector cc;
  cc = N_VNew_PSBLAS(NV_CCTXT_P(u),NV_DESCRIPTOR_P(u));
  N_VAsb_PSBLAS(cc);
  N_VAsb_PSBLAS(fval);
               ...
               void *user_data, N_Vector tmp1, N_Vector tmp2)
{
  ...
  // Who are we?
  psb_c_info(*(SM_CCTXT_P(J)),&iam,&np);
  if(iam == 0){
