...
/* This function returns the evaluation fval = Φ(u;parameters) to march the
   ...
   for (i=0; i<nl;  i++) {
     ...
     // entry of the funciton
      ...
      val[0] = (rho*phi/dt)*(Sfun(entries[1],alpha,n,thetas,thetar)
       -Sfun(entries[0],alpha,n,thetas,thetar))
  ...
  for (i=0; i<nl;  i++) {
   ...
   Kfunprime(entries[6],alpha,n,Ks)/deltahz2;
    ...
    val[el] = upstream(entries[1],entries[2],user_data)/sqdeltahx +
      ...
      (phi*rho*Sfunprime(entries[1],alpha,n,thetas,thetar))/dt;
    ijkinsert[0]=ix; ijkinsert[1]=iy; ijkinsert[2]=iz;
    icol[el] = psb_c_l_ijk2idx(ijkinsert,sizes,3,0);
    el=el+1;
    /*  term depending on     (i+1,j,k)        */
...
static double Sfun(double p, double alpha, double n, double thetas, double thetar){
  ...
  if (p >= 0.0){
    ...
    s = thetar + (-thetar + thetas)*pow(1. + alpha*SUNRabs(p),-1. + 1./n);
  }
...
static double Kfun(double p, double alpha, double n, double Ks){
  ...
  if (p >= 0.0){
     ...
     K = (Ks*(1. - pow(alpha,-1. + n)*pow(SUNRabs(p),-1. + n)*
        pow(1. + pow(alpha,n)*pow(SUNRabs(p),n),-1. + 1./n)))/
        pow(1. + pow(alpha,n)*pow(SUNRabs(p),n),0.5*(1. - 1./n));
  }

  return(K);

}
static double Sfunprime(double p, double alpha, double n, double thetas, double thetar){
